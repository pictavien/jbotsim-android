...
public class Forces {
	...
	public static ForceVector getSpringForce(Node from, Node to){
		...
		double direction=(distance>Dth)?Angles.getAngle(to, from):Angles.getAngle(from, to);
	...
	public static ForceVector getAngularForces(Node from, Node me){
		...
		if (selection.size()<6){
			...
			for (Node ng1 : selection){
				...
				double angle=Angles.getAngle(ng1, from, ng2);
		...
		for (Node ng1 : anglesLefts){
			...
			double dir1=Angles.getAngle(ng1, from)-(Math.PI)/2.0+0.45;
			double dir2=Angles.getAngle(ng2, from)+(Math.PI)/2.0-0.45;
	...
	private static double getAngularCorrection(Node master, Node slave1, Node slave2){
		double angle = Angles.getAngle(slave1, master, slave2);
